...
void ModelLoader::DispatchRead(FILE* apFile)
{
	...
	fscanf_s(apFile, "%s", tBuffer, _countof(tBuffer));
...
void ModelLoader::ReadVertexData(FILE* apFile)
{
    ...
    fscanf_s(apFile, "%f %f %f",&tmpVec.X(),&tmpVec.Y(),&tmpVec.Z());
...
void ModelLoader::ReadTextureData(FILE *apFile)
{
    ...
    fscanf_s(apFile, "%f %f %f",&tmpUV.X(),&tmpUV.Y(),&Unused);
...
void ModelLoader::ReadSurfaceData(FILE* apFile)
{
	...
	const char* delim = " ";
	char* next_token;
    TripletBuffer = strtok_s(tLineBuffer,delim, &next_token);
    while(TripletBuffer)
    {
        int i = 0;
        int j = 0;
        ...
        TripletBuffer = strtok_s(NULL,delim, &next_token);
